...
# ДОБАВЛЕНО: Словарь блокировок для защиты от race condition
_game_locks = {}
...
async def cmd_start_game(message: Message):
    ...
    # ИСПРАВЛЕНИЕ: блокировка от одновременного создания игры
    async with get_game_lock(chat_id):
        # Проверяем состояние игры
        if chat_id in active_games:
            game = active_games[chat_id]
            # Если игра завершена - удаляем
            if game.state == GameState.FINISHED:
                logger.info(f"Удаляем завершённую игру в чате {chat_id}")
                game.stop_timer()
                del active_games[chat_id]
            else:
                await message.answer("⚠️ Игра уже идёт! Используйте /status или /reset для сброса")
                return
        ...
        if not game.add_player(host_player):
            await message.answer("⚠️ Не удалось добавить игрока!")
            del active_games[chat_id]
            return
        ...
        logger.info(f"Создана новая игра в чате {chat_id}, хост: {message.from_user.username}")
...
async def join_game_callback(callback: CallbackQuery):
    ...
    if not game.add_player(player):
        await callback.answer("⚠️ Игра переполнена!", show_alert=True)
        return
...
async def start_game_callback(callback: CallbackQuery):
    ...
    logger.info(f"Игра началась в чате {chat_id}, шпион: {spy_id}, локация: {location['name']}")
...
async def cmd_cancel(message: Message):
    ...
    async with get_game_lock(chat_id):
        ...
        game.stop_timer()
        del active_games[chat_id]
        logger.info(f"Игра отменена в чате {chat_id}")
...
async def cmd_reset(message: Message):
    ...
    async with get_game_lock(chat_id):
        if chat_id in active_games:
            ...
            game.stop_timer()
            del active_games[chat_id]
            logger.info(f"Игра сброшена админом в чате {chat_id}")
            await message.answer("✅ Игра сброшена! Можете начать заново с /start")
        else:
